function d = SurfStatReadVol3( file, Z, T );
...
% BJA/RJA, 25 September 2019:
% Adding CIVM's beautiful 'load_niigz' function to support operating
% directly on gzipped nii's without having to gunzip first.
% Unsure of what, if any, modifications 'alx' implemented in
% SurfStatReadVol1.m...

% BJA/RJA, 8 October 2019:
% Changing behavorior such that nii headers are handled the exact same
% way, regardless of the data is gzipped on disk or not.
...
% This replaces code directly above. Still need proper ext.
if strcmp(ext,'.gz')
    [path,name,ext]=fileparts(name);    
end
...
if strcmp(ext,'.gz')
    try
        %d = civm_load_nii_gz(file,Z,T);
        if exist('T','var')
            new_data=civm_load_nii_gz(file,Z,T);
        elseif exist('Z','var')   
            new_data=civm_load_nii_gz(file,Z);
        else
            new_data=civm_load_nii_gz(file);
        end  
        
        [~,nii_binaryh,~,~]=load_niigz_hdr(file);
    catch
        if isunix
          unix(['gunzip ' file]);
          file(end-2:end)=[]; % Remove .gz from file name;
        else
          ['Can''t gunzip on non-unix system']
          return
        end
...
if ~exist('nii_binaryh','var')
    ...
    for i=1:length(machineformats)
      ...
      if sizeof_hdr==348;
         %Must have found a form that works, so break and continue using this format
         break;
      else
...
qform_code =fread(fid,1,'integer*2');
%sform_code =fread(fid,1,'short');
sform_code =fread(fid,1,'integer*2');
%quatern_b =fread(fid,1,'float');
quatern_b =fread(fid,1,'float32');
%quatern_c =fread(fid,1,'float');
quatern_c =fread(fid,1,'float32');
%quatern_d =fread(fid,1,'float');
quatern_d =fread(fid,1,'float32');
%qoffset_x =fread(fid,1,'float');
qoffset_x =fread(fid,1,'float32');
%qoffset_y =fread(fid,1,'float');
qoffset_y =fread(fid,1,'float32');
%qoffset_z =fread(fid,1,'float');
qoffset_z =fread(fid,1,'float32');
%srow_x =fread(fid,4,'float');
srow_x =fread(fid,4,'float32');
%srow_y =fread(fid,4,'float');
srow_y =fread(fid,4,'float32');
%srow_z =fread(fid,4,'float');
srow_z =fread(fid,4,'float32');
...
d.precision=[];
for ii=1:5
   if datatype==codes(ii)
      d.precision=deblank(types(ii,:));
   end
end
...
function [new_data] = civm_load_nii_gz(file,Z,T)
%function [d] = civm_load_nii_gz(file,Z,T)
% This code originally would handle the entire structure;
% But has been slightly edited to return only data matrix
'Attempting to use load_niigz function to load data.'
    a = load_niigz(file);
    afield = fields(a.hdr);
    d = a.hdr.(afield{1});
    for aa = 2:numel(afield);
         structA=a.hdr.(afield{aa});
         if isstruct(structA)
    ...
    if dim(1)==5
       if dim(5)==1 
          dim=[4; dim(2:4); dim(6); zeros(3,1)];
       else % The else part might be unneeded in this adaptation
          dim
          %fclose(fid);
          return
       end
    ...
    if d.qform_code>0;
        ...
        d.origin    = [d.srow_x(4) d.srow_y(4) d.srow_z(4)];
    end
    
    d.parent_file=file;

    if ( nargin<2 ) && ( ~exist('Z','var') )
       Z=1:d.dim(3);
    end

    if ( nargin<3 ) && ( ~exist('T','var') )
       T=1:d.dim(4);
    end
    ...
    if ischar(Z) && ischar(T)
        new_data=eval(['data(:,:,' Z ',' T ')']);
    elseif ischar(Z)
        new_data=eval(['data(:,:,' Z ',T)']);
    elseif ischar(T)
        new_data=eval(['data(:,:,Z,' T ')']);
    ...
    % if d.scl_slope~=0
    %    d.data=d.data*d.scl_slope+d.scl_inter;
    % end
    return
end
