function info=nifti1(thing,query)
% info=nifti1('data_type'|'type_code','value')
% Starter for a nifti matlab class. 
% H file pulled directly from.
% initially used to just decode image codes. 
% %% http://nifti.nimh.nih.gov/pub/dist/src/niftilib/nifti1.h
%
% March 11th, 2016
% - James Cook
%
% Current purpose is to serve as bridge of nifti types and matlab types. 
% (Its actually nicer than just matlab_types, converting other words into
% matlab types.)
% eg.
% nifti_type_code = nifti1('type_code', matlab_type);
% matlab_type     = nifti1('data_type', nifti_type_code);
...
data_type.DT_UNKNOWN =           0; %     /* what it says, dude           */
% #define DT_BINARY                  1     /* binary (1 bit/voxel)         */
% #define DT_UNSIGNED_CHAR           2     /* unsigned char (8 bits/voxel) */
data_type.DT_UNSIGNED_CHAR =     2; %     /* unsigned char (8 bits/voxel) */
% #define DT_SIGNED_SHORT            4     /* signed short (16 bits/voxel) */
data_type.DT_SIGNED_SHORT =      4; %     /* signed short (16 bits/voxel) */
% #define DT_SIGNED_INT              8     /* signed int (32 bits/voxel)   */
data_type.DT_SIGNED_INT =        8; %     /* signed int (32 bits/voxel)   */
% #define DT_FLOAT                  16     /* float (32 bits/voxel)        */
data_type.DT_FLOAT =            16; %     /* float (32 bits/voxel)        */
% #define DT_COMPLEX                32     /* complex (64 bits/voxel)      */
data_type.DT_COMPLEX =          32; %     /* complex (64 bits/voxel)      */
% #define DT_DOUBLE                 64     /* double (64 bits/voxel)       */
data_type.DT_DOUBLE =           64; %     /* double (64 bits/voxel)       */
% #define DT_RGB                   128     /* RGB triple (24 bits/voxel)   */
data_type.DT_RGB =             128; %     /* RGB triple (24 bits/voxel)   */
% #define DT_ALL                   255     /* not very useful (?)          */
data_type.DT_ALL =             255; %     /* not very useful (?)          */
% 
%                             /*----- another set of names for the same ---*/
% #define DT_UINT8                   2
data_type.DT_UINT8 =             2; %
% #define DT_INT16                   4
data_type.DT_INT16 =             4; %
% #define DT_INT32                   8
data_type.DT_INT32 =             8; %
% #define DT_FLOAT32                16
data_type.DT_FLOAT32 =          16; %
% #define DT_COMPLEX64              32
data_type.DT_COMPLEX64 =        32; %
% #define DT_FLOAT64                64
data_type.DT_FLOAT64 =          64; %
% #define DT_RGB24                 128
data_type.DT_RGB24 =           128; %
% 
%                             /*------------------- new codes for NIFTI ---*/
% #define DT_INT8                  256     /* signed char (8 bits)         */
data_type.DT_INT8 =            256; %     /* signed char (8 bits)         */
% #define DT_UINT16                512     /* unsigned short (16 bits)     */
data_type.DT_UINT16 =          512; %     /* unsigned short (16 bits)     */
% #define DT_UINT32                768     /* unsigned int (32 bits)       */
data_type.DT_UINT32 =          768; %     /* unsigned int (32 bits)       */
% #define DT_INT64                1024     /* long long (64 bits)          */
data_type.DT_INT64 =          1024; %     /* long long (64 bits)          */
% #define DT_UINT64               1280     /* unsigned long long (64 bits) */
data_type.DT_UINT64 =         1280; %     /* unsigned long long (64 bits) */
% #define DT_FLOAT128             1536     /* long double (128 bits)       */
data_type.DT_FLOAT128 =       1536; %     /* long double (128 bits)       */
% #define DT_COMPLEX128           1792     /* double pair (128 bits)       */
data_type.DT_COMPLEX128 =     1792; %     /* double pair (128 bits)       */
% #define DT_COMPLEX256           2048     /* long double pair (256 bits)  */
data_type.DT_COMPLEX256 =     2048; %     /* long double pair (256 bits)  */
% #define DT_RGBA32               2304     /* 4 byte RGBA (32 bits/voxel)  */
data_type.DT_RGBA32 =         2304; %     /* 4 byte RGBA (32 bits/voxel)  */
% /* @} */
% 
% 
...
data_type.NIFTI_TYPE_UINT8 =          2;
%                                        /*! signed short. */
% #define NIFTI_TYPE_INT16           4
data_type.NIFTI_TYPE_INT16 =          4;
%                                        /*! signed int. */
% #define NIFTI_TYPE_INT32           8
data_type.NIFTI_TYPE_INT32 =          8;
%                                        /*! 32 bit float. */
% #define NIFTI_TYPE_FLOAT32        16
data_type.NIFTI_TYPE_FLOAT32 =       16;
%                                        /*! 64 bit complex = 2 32 bit floats. */
% #define NIFTI_TYPE_COMPLEX64      32
data_type.NIFTI_TYPE_COMPLEX64 =     32;
%                                        /*! 64 bit float = double. */
% #define NIFTI_TYPE_FLOAT64        64
data_type.NIFTI_TYPE_FLOAT64 =       64;
%                                        /*! 3 8 bit bytes. */
% #define NIFTI_TYPE_RGB24         128
data_type.NIFTI_TYPE_RGB24 =        128;
%                                        /*! signed char. */
% #define NIFTI_TYPE_INT8          256
data_type.NIFTI_TYPE_INT8 =         256;
%                                        /*! unsigned short. */
% #define NIFTI_TYPE_UINT16        512
data_type.NIFTI_TYPE_UINT16 =       512;
%                                        /*! unsigned int. */
% #define NIFTI_TYPE_UINT32        768
data_type.NIFTI_TYPE_UINT32 =       768;
%                                        /*! signed long long. */
% #define NIFTI_TYPE_INT64        1024
data_type.NIFTI_TYPE_INT64 =       1024;
%                                        /*! unsigned long long. */
% #define NIFTI_TYPE_UINT64       1280
data_type.NIFTI_TYPE_UINT64 =      1280;
%                                        /*! 128 bit float = long double. */
% #define NIFTI_TYPE_FLOAT128     1536
data_type.NIFTI_TYPE_FLOAT128 =    1536;
%                                        /*! 128 bit complex = 2 64 bit floats. */
% #define NIFTI_TYPE_COMPLEX128   1792
data_type.NIFTI_TYPE_COMPLEX128 =  1792;
%                                        /*! 256 bit complex = 2 128 bit floats */
% #define NIFTI_TYPE_COMPLEX256   2048
data_type.NIFTI_TYPE_COMPLEX256 =  2048;
%                                        /*! 4 8 bit bytes. */
% #define NIFTI_TYPE_RGBA32       2304
data_type.NIFTI_TYPE_RGBA32 =      2304;
% /* @} */
% 
...
% data_type_matlab.val_2='char';
data_type_matlab.val_2='uint8';
data_type_matlab.val_4='int16';
data_type_matlab.val_8='int32';
data_type_matlab.val_16='single';
data_type_matlab.val_32='complex';
data_type_matlab.val_64='double';
data_type_matlab.val_128='rgb';
% rgb datatype is intentionally NOT a matlab type.
% data_type_matlab.val_128='uint8';
data_type_matlab.val_255='all';
data_type_matlab.val_256='int8';
data_type_matlab.val_512='uint16';
data_type_matlab.val_768='uint32';
data_type_matlab.val_1024='int64';
data_type_matlab.val_1280='uint64';
data_type_matlab.val_1536='float128';
data_type_matlab.val_1792='complex128';
data_type_matlab.val_2048='complex256';
% rgb32 datatype is intentionally NOT a matlab type.
data_type_matlab.val_2304='rgba32';
% data_type_matlab.val_2304='uint32';
if strcmp(thing,'datatype')
   thing = 'data_type'; 
end
if strcmp(thing,'data_type')
    info=data_type_matlab.(sprintf('val_%i',query));
elseif strcmp(thing,'type_code')
    fields=fieldnames(data_type);
    found=find(~cellfun(@isempty,regexpi(fields,['^.*' query '*$']))');
    if numel(found)>0
        info=data_type.(fields{found(end)});
    else
        fields=fieldnames(data_type_matlab);
        mtypes=struct2cell(data_type_matlab);
        found=find(~cellfun(@isempty,regexpi(mtypes,['^.*' query '*$']))');
        t=regexpi(fields{found(end)},'^val_(.+)$','tokens');
        info=str2double(t{1}{1});
    end
end
end    